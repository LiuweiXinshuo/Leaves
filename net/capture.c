...
/* 物理网卡接口 */
static const char *g_szIfName = "ens33";
...
static const char g_szProName[][24] = {"none", "xernx pup", "sprite", "rarp", "apple-protocol", "apple-arp", "802.1q", "ipx", "ipv6", "loopback"};
...
static int ethdump_setPromisc(const char *pcIfName, int fd, int iFlags)
{
	...
	if(0 > iRet)
	{
		perror("[Error]Get Interfer Flags");
		return -1;
	}
	if(0 == iFlags)
		stIfr.ifr_flags &= ~IFF_PROMISC;	/* 取消 */
	else
		stIfr.ifr_flags |= IFF_PROMISC;		/* 设置 */
